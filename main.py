...
# load databases===============
sym_des = pd.read_csv('medical_set/symtoms_df.csv')
precautions = pd.read_csv('medical_set/precautions_df.csv')
workout = pd.read_csv('medical_set/workout_df.csv')
description = pd.read_csv('medical_set/description.csv')
medications = pd.read_csv('medical_set/medications.csv')
diets = pd.read_csv('medical_set/diets.csv')
# load models=============
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            ...
            raw_user_symptoms = [s.strip() for s in symptoms.split(',')]
            raw_user_symptoms = [symptom.strip("[]' ") for symptom in raw_user_symptoms]
            user_symptoms = []
            for symp in raw_user_symptoms:
                for elem in symptoms_dict.keys():
                    if fuzz.ratio(elem, symp) > ACCEPTED_RATIO:
                        user_symptoms.append(elem)
            predicted_diseases = get_predicted_value(user_symptoms)
            dis_des, precaution, medication, rec_die, workout = helper(predicted_diseases)
            print(f'{dis_des=}, {precaution=}, {medication=}, {rec_die=}, {workout=}')
        my_precautions = []
        for i in precaution[0]:
            my_precautions.append(i)

            my_medications = []
            for j in medication[0]:
                my_medications.append(j)

        return render_template('index.html', predicted_disease=predicted_diseases, dis_des=dis_des, my_precautions=my_precautions,
                               medications=medication[0][2:-2].split(','), workout=workout, my_diet=rec_die)
    return render_template('index.html')
